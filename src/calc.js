...
export function somaRendimentosCategoriaA(linhasAnexoA) {
...
export function deducaoEspecificaCategoriaA(contribuicoesObrigatorias, parametros) {
  return Math.max(contribuicoesObrigatorias, parametros.deducaoEspecificaCategoriaA.minimo);
}

export function aplicarEscaloes(rendimentoColetavel, escaloes) {
  let imposto = 0;
...
export function deducaoPorDependentes(dependentes, parametros) {
...
export function calcularEstimativa({ agregado, anexoA, parametros }) {
